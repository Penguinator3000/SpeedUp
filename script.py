...
#Speed up the PC (Carlos Hacks :D)
...
#Clear Browser Data of Chrome, IE, Opera, Firefox

# Paths to the cache directories for each browser
paths = {
    ...
    'ie': r'%LOCALAPPDATA%\Microsoft\Windows\INetCache'
}

for browser, path in paths.items():
    resolved_path = os.path.expandvars(path)
    if os.path.exists(resolved_path):
        print(f'Deleting cache for {browser}...')
        shutil.rmtree(resolved_path)
    else:
        print(f'{browser} is not installed, skipping...')

#Update all (Carlos Hacks :D)

def install_updates_and_restart():
    # Define the PowerShell command that will install the PSWindowsUpdate module
    install_module_cmd = 'powershell.exe "Install-Module PSWindowsUpdate -Force -Confirm:$false"'

    # Execute the command to install the PSWindowsUpdate module
    process = subprocess.Popen(install_module_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    # Define the PowerShell command that will install all updates
    install_updates_cmd = 'powershell.exe "Install-WindowsUpdate -MicrosoftUpdate -AcceptAll -AutoReboot"'

    # Execute the command to install updates
    process = subprocess.Popen(install_updates_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout, stderr = process.communicate()
